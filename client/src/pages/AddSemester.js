...
const AddSemester = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const addSemester = await api.post('semester/create', data, 
            {headers: headers}
            )
        ...
        <div className="container">
            <Grid container spacing={2} className="gridHolder">
                <Grid item xs={3} ml="auto" mr="auto">
                    <Typography variant="h6" sx={{ textAlign: 'center' }} pb={2} color="primary">Add a Semester</Typography>
                        ...
                        {(!submitted || sError) && (
                            <form onSubmit={handleSubmit}>
                                ...
                                <Select
                                    ...
                                    value={data.nameSemester}
                                    name="nameSemester"
                                    onChange={handleChange}
                                    sx={{width:"90%"}}
                                >
                                    {semesterList.map((e) => <MenuItem value={e}>{e}</MenuItem>)}
                                </Select>                                </div>
                                <Button variant="contained" type="submit" color="primary" sx={{ width: '50%' }}>Add Semester</Button>
                            </form>
                        )}
                        {(submitted && !sError) && (
                        <><Typography variant="h6" color="success">Semester was added successfully !  </Typography><Button sx={{margin:2}}color="success" variant="contained" onClick={() => {setSubmitted(false)}}>Add Another Semester</Button></>
                        )}
                    </Paper>
                </Grid>
            </Grid>
